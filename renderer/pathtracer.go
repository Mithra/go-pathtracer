	...
	for yOffset := 0; yOffset < 2; yOffset++ {
		for xOffset := 0; xOffset < 2; xOffset++ {
			...
			for subSample := 0; subSample < nbSamples; subSample++ {
				...
				pixelNdcX := ((dx+float64(xOffset))/2 + float64(x) + 0.5) / float64(options.Width)
				pixelNdcY := ((dy+float64(yOffset))/2 + float64(y) + 0.5) / float64(options.Height)
				...
				acc = acc.Add(r.radiance(ray, scene, options, 0, randomSeed, true).MulScalar(1. / float64(nbSamples)))
			}
...
func (r PathTracer) radiance(ray Ray, scene Scene, options RenderingOptions, depth uint, randomSeed []uint, includeLights bool) Vector3 {
	depth = depth + 1
	...
	if depth > 5 || p == 0 {
		if erand48(randomSeed) < p {
			...
			return defaultColor // TODO: obj.e * E ?
		}
	}
	...
	if material.Reflectivity == 0 && material.Transparency == 0 {
		...
		for i := 0; i < len(scene.Lights); i++ {
			...
			if transmission {
				...
				e = e.Add(objectColor.Mul(light.EmissionColor.MulScalar(lightDirection.Dot(nhit) * omega)).MulScalar(M_1_PI))
			}
		}
		...
		return e
		return e.Add(objectColor.Mul(r.radiance(NewRay(phit, d), scene, options, depth, randomSeed, false)))
	}
	...
	if material.Transparency == 0 {
		// TODO : ?
		return objectColor.Mul(r.radiance(reflectionRay, scene, options, depth, randomSeed, true))
	}
	...
	if cost2t < 0 {
		return objectColor.Mul(r.radiance(NewRay(phit, reflectionDirection), scene, options, depth, randomSeed, true))
	}
	...
	// 	// Russian Roulette
	// 	if erand48(randomSeed) < P {
	// 		return r.radiance(reflectionRay, scene, options, depth, randomSeed, true).MulScalar(Rp)
	// 	}
	// 	return r.radiance(NewRay(phit, tdir), scene, options, depth, randomSeed, true).MulScalar(TP)
	// }

	// c1 := r.radiance(reflectionRay, scene, options, depth, randomSeed, true).MulScalar(Re)
	// c2 := r.radiance(NewRay(phit, tdir), scene, options, depth, randomSeed, true).MulScalar(Tr)
